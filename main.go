...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Println("\nExample:")
		fmt.Println("  odr-webapi -port 9000 -target /dabplus/dls/dls.txt")
	}
	...
	if *port <= 0 || *target == "" {
		fmt.Println("Error: All parameters (port, target) are mandatory")
		flag.Usage()
		os.Exit(1)
	}
	...
	http.HandleFunc("/api/dls", func(w http.ResponseWriter, r *http.Request) {
	...
	http.HandleFunc("/api/status", func(w http.ResponseWriter, r *http.Request) {
		lastUpdateTime := getLastUpdateTime(config.TargetPath)
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprintf(w, `{"status":"online","target":"%s","lastUpdate":"%s"}`, config.TargetPath, lastUpdateTime)
	})
